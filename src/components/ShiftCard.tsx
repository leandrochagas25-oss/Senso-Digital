...
interface ShiftCardProps {
  ...
  onTransfer: (shift: ShiftHandover) => void;
  onHomeDischarge: (shift: ShiftHandover) => void;
  onDeath: (shift: ShiftHandover) => void;
  canEdit?: boolean;
}

export const ShiftCard: React.FC<ShiftCardProps> = ({ shift, onEdit, onTransfer, onHomeDischarge, onDeath, canEdit = true }) => {
  const age = shift.birthDate ? differenceInYears(new Date(), parseISO(shift.birthDate)) : 'N/I';
  const hasPending = (shift.pending && shift.pending.trim().length > 0) || (shift.pendingExams && shift.pendingExams.length > 0);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
        ...
        {/* New Fields Summary */}
        <div className="md:col-span-4 grid grid-cols-2 md:grid-cols-6 gap-2 mt-1 pt-2 border-t border-slate-100">
          ...
          <div className="flex flex-col">
            <span className="text-[8px] font-bold text-slate-400 uppercase">Exames</span>
            <span className={cn(
              "text-[10px] font-bold truncate",
              hasPending ? "text-red-600" : "text-slate-600"
            )}>
              {shift.pendingExams && shift.pendingExams.length > 0 ? `${shift.pendingExams.length} pendentes` : 'Nenhum'}
            </span>
          </div>
          <div className="flex flex-col">
          ...
          {(shift.vasoactiveDrugs || (shift.vasoactiveDrugsList && shift.vasoactiveDrugsList.length > 0)) && (
            <div className="flex flex-col">
              ...
              <div className="flex flex-wrap gap-1">
                {shift.vasoactiveDrugsList && shift.vasoactiveDrugsList.length > 0 ? (
                  shift.vasoactiveDrugsList.map((drug, idx) => (
                    <span key={idx} className="text-[9px] text-medical-red font-bold bg-red-50 px-1 rounded border border-red-100">
                      {drug.name}: {drug.dose} ml/h
                    </span>
                  ))
                ) : (
      ...
      {canEdit && (
        <div className="flex flex-col gap-2 ml-2">
          ...
          <button 
            onClick={(e) => {
              e.stopPropagation();
              onHomeDischarge(shift);
            }}
            className="px-4 py-2 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-emerald-700 transition-all shadow-md shadow-emerald-200 active:scale-95"
          >
            ALTA
          </button>
          <button 
